...
<cfparam name="url.uyari" default="">
<cfparam name="form.csrf" default="">
<cfset csrfGecerli=compare(form.csrf,SESSION.csrf) EQ 0>

<cfif url.uyari EQ "hizli">
    <cfset hata="Çok sık yorum gönderiyorsunuz.Lütfen birkaç saniye bekleyip tekrar deneyiniz.">
<cfelseif url.uyari EQ "tekrar">
    <cfset hata="Aynı yorumu kısa süre içinde tekrar gönderemezsiniz.">
<cfelseif url.uyari EQ "csrf">
...
<cfif structKeyExists(form,"cevapDuzelt") AND val(SESSION.rol) GTE 2>
    ...
    <cfif NOT csrfGecerli>
        <cfset cevapHata="Oturum doğrulaması başarısız.Sayfayı yenileyip tekrar deneyiniz.">
    <cfelseif NOT listFind("A,B,C,D,E",yeniCevap)>
...
<cfset goruntulenme=val(qSoru.goruntulenmeSayisi)>
<cfif NOT structKeyExists(SESSION,"goruntulenenSorular") OR NOT isStruct(SESSION.goruntulenenSorular)>
    <cfset SESSION.goruntulenenSorular={}>
</cfif>

<cfif cgi.request_method NEQ "POST" AND qSoru.soranID NEQ kullaniciID AND NOT structKeyExists(SESSION.goruntulenenSorular,soruID)>
    <cfquery datasource="DSN">
        UPDATE Soru
        SET goruntulenmeSayisi=goruntulenmeSayisi+1
        WHERE id=<cfqueryparam value="#soruID#" cfsqltype="cf_sql_integer">
    </cfquery>
    <cfset SESSION.goruntulenenSorular[soruID]=true>
    <cfset goruntulenme=goruntulenme+1>
</cfif>
...
<cfif structKeyExists(form,"cevapGonder") AND NOT cevapKontrol>
    ...
    <cfif NOT csrfGecerli>
        <cfset hata="Oturum doğrulaması başarısız.Sayfayı yenileyip tekrar deneyiniz.">
    <cfelseif qSoru.soranID EQ kullaniciID>
        <cfset hata="Kendi sorunuzu çözerek puan elde edemezsiniz">
    ...
    <cfelse>
        ...
        <cftry>
            ...
            <cfcatch type="any">
                <cfif structKeyExists(cfcatch,"nativeErrorCode") AND listFind("2601,2627",val(cfcatch.nativeErrorCode))>
                    <cfset hata="Bu soruyu zaten cevapladınız.">
                    <cfquery name="qKontrol" datasource="DSN">
                        SELECT id,kullaniciCevabi,onay
                        FROM Cevap
                        WHERE soruID=<cfqueryparam value="#soruID#" cfsqltype="cf_sql_integer">
...
<cfquery name="qCevaplar" datasource="DSN">
    SELECT c.id,c.cozenID,c.cozumMetni,c.onay,c.eklenmeTarihi,
    k.ad AS cozenAd,
    ...
    WHERE c.soruID=<cfqueryparam value="#soruID#" cfsqltype="cf_sql_integer">
    AND c.aktiflik=1
    AND NULLIF(LTRIM(RTRIM(c.cozumMetni)),'') IS NOT NULL
...
<cfset geriURL="/YKSSite/views/soru/soruDetay.cfm?id=" & soruID>
        ...
        <div class="yigin">
            <article class="kart">
                <div class="kart__baslik">
                    ...
                    <span class="veri sessiz">Görüntülenme Sayısı:#goruntulenme#</span>
                </div>
                ...
                <div class="kart__ayrac">
                    <div class="eylem-seridi">
                        <span class="mini-avatar">#encodeForHTML(uCase(left(qSoru.soranAd,2)))#</span>
            ...
            <cfif NOT cevapKontrol>
                <cfif qSoru.soranID EQ kullaniciID>
                    <div class="bildirim" role="status">Kendi sorunuzu çözerek puan elde edemezsiniz</div>
                <cfelse>
                    <section class="kart">
            ...
            <cfif cevapKontrol AND qCevaplar.recordCount GT 0>
                <h2 class="goz">Çözümler · #qCevaplar.recordCount#</h2>
                <cfloop query="qCevaplar">
                    <article class="kart">
                        <div class="kart__govde">
                            <div class="cozum-kart__ust">
                                <span class="mini-avatar">#encodeForHTML(uCase(left(qCevaplar.cozenAd,2)))#</span>
                            ...
                            <div class="yorum-alani" id="yorumlar_#qCevaplar.id#" hidden>
                                <cfif structKeyExists(ustYorumlar,val(qCevaplar.id))>
                                    <cfloop array="#ustYorumlar[val(qCevaplar.id)]#" index="y">
                                        <div class="yorum">
                                            <span class="avatar">#encodeForHTML(uCase(left(y.yazar,2)))#</span>
                                                ...
                                                <cfif structKeyExists(yanitlar,y.id)>
                                                    <cfloop array="#yanitlar[y.id]#" index="yanit">
                                                        <div class="yorum yorum--yanit">
                                                            <span class="mini-avatar">#encodeForHTML(uCase(left(yanit.yazar,1)))#</span>
                                                            <div style="flex:1;min-width:0">
                                                                <div class="yorum__ust">
            ...
            <cfelseif NOT cevapKontrol>>
                <div class="bildirim" role="status">Çözümleri görmek için öncelikle soruyu çözmelisiniz</div>
            </cfif>
        </div>
        <div class="yigin">
            <section class="kart">
                ...
                <div class="kart__govde">
                    <dl class="bilgi-liste">
                        ...
                        <div><dt>Görüntülenme:</dt><dd class="veri">#goruntulenme#</dd></div>
                    </dl>
                </div>
            </section>
            <cfif aiCozumGoster>
                <button class="dugme dugme--ikincil dugme--tam" type="button" id="aiAcDugme" data-pencere-ac="aiPencere" data-soruid="#qSoru.id#" data-csrf="#encodeForHTMLAttribute(SESSION.csrf)#">Yapay zeka ile çöz</button>
            </cfif>
                    ...
                    <div class="kart__govde">
                        ...
                        <form method="POST" action="?id=#soruID#">
                            ...
                            <div class="alan ust-bosluk">
                                ...
                                <span class="alan__ipucu">Mevcut doğru cevap:#encodeForHTML(qSoru.dogruCevap)# <br>Değiştirirseniz tüm cevaplar ve puanlar yeniden hesaplanır</span>
                            </div>
